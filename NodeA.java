...
public class NodeA extends Node_Graph{
	static int settings = -1;
	
	public NodeA() { super(); }
	
	public NodeA(int settings) { this.setSettings(settings); }
	
	public NodeA(int id, String name) { super(id, name); setSettings(NodeA.settings); }
	
	public NodeA(int id, String name, int settings, BufferedWriter log_writer) { 
		super(id, name); this.cast(settings, log_writer); 
	}
	/**
	 *Set settings for node types NodeA
	 *usage:
	 ...
	 * @param type settings for NodeA
	 */
	public void setSettings(int type){
		if(type > 3 || type < 0)
			return;
		if(NodeA.settings == type)
			NodeA.settings = -1;
		if(NodeA.settings == -1)
			switch(type){
			case 1:
			...
			if(type < NodeA.settings)
				return;
				switch(NodeA.settings){
				case 1:
					if(type == 2){
	...
	public void cast(int type, BufferedWriter log_writer){
		...
		if(type < NodeA.settings){
			try {
				log_writer.write("Fail cast NodeA" + " " + this.getName() + " "+ "from Version=");
				log_writer.write( NodeA.settings + " " + "to Version="+ '"' + type + '"');
				log_writer.newLine();
			} catch (IOException e) {
		...
		if(NodeA.settings == type){
			...
			try {
				log_writer.write("Ok cast NodeA" + " " + this.getName() + " "+ "from Version=");
				log_writer.write('"'+  NodeA.settings+ '"'+"to Version="+ '"' +type + '"');
				log_writer.newLine();
			} catch (IOException e) {
				System.out.println("Interrupted I/O operation");
